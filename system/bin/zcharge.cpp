...
void set_charging_switch(const string &switch_) {
  ...
  charging_switch_value = switch_;

  // Give the kernel a moment to apply the new state.
  for (int i = 0; i < wait_time; ++i) {
    ...
    if (actual_value == switch_) {
      ALOGI("Charging switch changed to %s", switch_.c_str());
      ALOGD("Battery current is %dµA", read_current_now());
      return;
    }
  ...
  ALOGW("Charging switch did not confirm value %s", switch_.c_str());
  ALOGD("Battery current is %dµA", read_current_now());
}
...
void limiter_service(const string &db_file) {
  ...
  try {
    ...
    while (enabled) {
      ...
      // Read the actual charging switch state.
      ...
      // Capacity controller
      //
      // ON  (0)  = charging allowed
      // OFF (1)  = charging suspended
      //
      // 50% -> suspend charging
      // <48% -> resume charging
      // ------------------------------------------------------------
      ...
      if (capacity >= capacity_limit &&
        ...
        ALOGI("Charging suspended until battery falls below %d%%",
              recharging_limit);
      ...
      //
      // Kept independent from USB presence.
      // The configured value is normally 800 (80°C), so it does
      ...
      this_thread::sleep_for(
          chrono::seconds(MAIN_LOOP_INTERVAL_SECONDS));
    }
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 4) {
    ...
    string db_file = (argc == 4) ? argv[2] : default_db_file;
    string key_value = (argc == 4) ? argv[3] : argv[2];
...
void limiter_service(const string &db_file) {
  ...
  // Initial configuration load
  int recharging_limit, capacity_limit, temp_limit, temperature;
  bool cooldown = true, cooling_off = false;
  string sql = "SELECT key, value FROM zcharge_config";
  sqlite3_stmt *stmt;
